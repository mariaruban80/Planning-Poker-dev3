...
let preservedManualTickets = []; // Track manually added tickets that need to be preserved
...
function updateUserList(users) {
  console.log('[APP] Updating user list with', users.length, 'users');
  
  const userListContainer = document.getElementById('userList');
  const userCircleContainer = document.getElementById('userCircle');
  
  if (!userListContainer || !userCircleContainer) {
    console.error('[APP] User list containers not found in DOM');
    return;
  }
...
/**
 * Normalize story indexes to ensure proper selection and navigation
 */
...
/**
 * Trigger a global emoji burst effect
 */
